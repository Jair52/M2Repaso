...
title.innerHTML = title.innerHTML + " PETER";
...
// const instanciaTest = new ToDo("correr")
// ToDo {complete = false, description = "correr"}
/*
* STEPS
Tomar el value del Input y Crear el objeto -> ToDo {complete = false, description = "correr"}
Pushear este objeto a -> toDoItems = [{complete = false, description = "correr"}]
Recorrer el array toDoItems y para cada objeto que tenemos creamos un element ->
        newArrayByElements    [{<div class="toDoShell"><span id=2 >correr</span></div>}]
Recorrer el newArrayByElements y por cada objeto lo apendeamos a:  <div id="toDoContainer">...</div>
                por defecto le asignamos .completeText {text-decoration: line-through;} <- false
...
// {complete = false, description = "correr"} ==> {<div class="toDoShell"><span id=2 >correr</span></div>}.completeToDo()
...
//                                                                 |
// -> toDoItems = [{complete = false, description = "correr"}, {complete = false, description = "saltar"}]
...
function displayToDos() {
  ...
  var result = buildToDos(toDoItems); // [{<id=0>}, {<id=1>}]
...
function addToDo() {
  ...
  var todo = new ToDo(toDoInput.value); // "caminar" -> {complete:false, description: "caminar"}
  if (todo.description === "") {
    alert("el campo está vacío");
  } else {
    toDoItems.push(todo); // -> [{complete:false, description: "caminar"}]
    toDoInput.value = "";
  }
  displayToDos();
}
...
/*
AGREGAR   ->   addToDo()  -> displayToDos()
ELEMENT en SÍ -> completeToDo()  ->  displayToDos()

    displayToDos() ->   toDoItems  <- buildToDos(toDoItems)
                                              buildToDo(element, index)
                                            para result [<>, <>]
                                            y a cada element apend a -> toDoContainer
                                            * aquí está lo que termina mostrando en pantalla


addToDo   data -> {description complete}

completeToDo    toDoItems   [     index -> complete a true]
*/
